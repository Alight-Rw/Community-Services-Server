...
const verifyAccount = async (req, res) => {
  try {
    ...
    if (!token) return handleError(res, StatusCodes.BAD_REQUEST, 'Token is required');
    const payload = jwt.verify(token, process.env.JWT_SECRET);
    const user = await User.findOne({ email: payload.email, verificationToken: token });
    if (!user) return handleError(res, StatusCodes.BAD_REQUEST, 'Invalid token');
    if (user.isVerified) return handleError(res, StatusCodes.BAD_REQUEST, 'Account already verified');
